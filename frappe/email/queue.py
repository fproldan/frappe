...
def prepare_message(email, recipient, recipients_list):
	...
	if frappe.conf.use_ssl and email_account.track_email_status:
		...
		message = message.replace("<!--email open check-->", quopri.encodestring('<img src="{}/api/method/frappe.core.doctype.communication.email.mark_email_as_seen?name={}"/>'.format(get_url(), email.communication).encode()).decode()).replace('\r\n', '').replace('=3D', '=')
	else:
