...
def prepare_message(email, recipient, recipients_list):
	...
	if frappe.conf.use_ssl and email_account.track_email_status:
		...
		site = frappe.conf.host_name or frappe.local.site
		message = message.replace("<!--email open check-->", quopri.encodestring('<img src="https://{}/api/method/frappe.core.doctype.communication.email.mark_email_as_seen?name={}"/>'.format(site, email.communication).encode()).decode())
	else:
